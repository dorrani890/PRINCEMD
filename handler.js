...
/*if (process.env.AUTOREAD === 'true') {
 ...
 // STATUSVIEW 
	    //if (typeof process.env.STATUSVIEW !== 'undefined' && process.env.STATUSVIEW.toLowerCase() === 'true') { if (m.key.remoteJid === 'status@broadcast') { await conn.readMessages([m.key]); } }
         ...
         let bot = global.db.data.settings[this.user.jid] || {}; if (process.env.STATUSVIEW && process.env.STATUSVIEW.toLowerCase() === 'true') { if (m.key.remoteJid === 'status@broadcast' && !m.fromMe) { await conn.readMessages([m.key]); const prince = ['🖤', '🇵🇰', '🥺', '🥺', '😊', '😇', '🙂', '🇵🇰', '🇵🇰']; const randomEmoji = prince[Math.floor(Math.random() * prince.length)]; const me = await conn.decodeJid(conn.user.id); await conn.sendMessage(m.key.remoteJid, { react: { key: m.key, text: randomEmoji } }, { statusJidList: [m.key.participant, me] }); } } else if (bot.statusview) { if (m.key.remoteJid === 'status@broadcast' && !m.fromMe) { await conn.readMessages([m.key]); const prince = ['🖤', '🇵🇰', '🥺', '🥺', '😊', '😇', '🙂', '🇵🇰', '🇵🇰']; const randomEmoji = prince[Math.floor(Math.random() * prince.length)]; const me = await conn.decodeJid(conn.user.id); await conn.sendMessage(m.key.remoteJid, { react: { key: m.key, text: randomEmoji } }, { statusJidList: [m.key.participant, me] }); } }



	    

if ((process.env.AutoReaction && process.env.AutoReaction.toLowerCase() === 'true') || (global.db.data.settings[this.user.jid]?.autoreacts)) { if (m.text.match(/(prince||e|i|o|u|g|q|ا|م|@| |0|9|8|7|6|5|4|3|2|1||.|b|c|d|e|f|g|h|i|j|k|l|m|n|o|p|q|r|s|t|u|v|w|x|y|z)/gi)) { this.sendMessage(m.chat, { react: { text: (m.sender === '923092668108@s.whatsapp.net') ? "🇵🇰" : pickRandom(["☺️", "😻", "🥰", "😱", "🤗", "🥺", "🤭", "☺️", "✨", "🎉", "💗", "♥️", "👑", "💞", "💖", "💓", "🥀", "🖤", "😘", "🫣", "😩", "😩", "❤️", "🧡", "💛", "💚", "💙", "💜", "🖤", "🤍", "💟", "🥺", "😎", "😍", "🕊️", "🥀", "🦋", "😒", "❤‍🩹", "♥️", "😒", "🌸", "😩", "❣️", "✨", "🙌", "👻", "🙃", "🪽", "🌙", "💫", "🙃", "🧸", "🎀", "🎉", "🩷", "🖤", "🤍", "🤎", "💛", "💚", "🩵", "💙", "💜", "💟", "💓", "🩶", "🥺", "🇵🇰", "🥺"]), key: m.key } }); } } function pickRandom(list) { return list[Math.floor(Math.random() * list.length)]; }
