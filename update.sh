...
if [[ $(basename "$PWD") == "$BOT_DIR" ]]; then 
    if [ -e "$DB_FILE" ]; then 
        ...
        mv "$HOME/$BOT_DIR/$DB_FILE" "$HOME" && cd && rm -rf "$HOME/$BOT_DIR" && git clone "$BOT_REPO" && cd "$HOME/$BOT_DIR" && yarn --ignore-scripts && npm install --legacy-peer-deps && cd
        ...
        cd && rm -rf "$HOME/$BOT_DIR" && git clone "$BOT_REPO" && cd "$HOME/$BOT_DIR" && yarn --ignore-scripts && npm install --legacy-peer-deps && cd
        if [ -e "$HOME/$DB_FILE" ]; then
            echo -e "${BOLD}${GREEN}Rescuing file \"$DB_FILE\" and moving it to \"$BOT_DIR\".\n${RESET}"
            echo -e "${BOLD}${GREEN}Starting $BOT_DIR...\n${RESET}"
            mv "$HOME/$DB_FILE" "$HOME/$BOT_DIR/" && cd "$BOT_DIR" && npm start
    ...
    if [ -e "$HOME/$BOT_DIR" ]; then
        ...
        if [ -e "$HOME/$BOT_DIR/$DB_FILE" ]; then
            ...
            mv "$HOME/$BOT_DIR/$DB_FILE" "$HOME" && cd && rm -rf "$BOT_DIR" && git clone "$BOT_REPO" && cd "$BOT_DIR" && yarn --ignore-scripts && npm install --legacy-peer-deps && cd
            if [ -e "$HOME/$DB_FILE" ]; then
                echo -e "${BOLD}${GREEN}Rescuing file \"$DB_FILE\" and moving it to \"$BOT_DIR\".\n${RESET}"
            ...
            cd && rm -rf "$BOT_DIR" && git clone "$BOT_REPO" && cd "$BOT_DIR" && yarn --ignore-scripts && npm install --legacy-peer-deps &&
            echo -e "${BOLD}${GREEN}Starting $BOT_DIR...\n${RESET}"
            npm start
        fi
    else
